...
//This is from the treehouse tutorial project on using OAuth to connect to Yelp.  
//When the 'requestOAuthToken()' is called oauth.authorize returns a "Could not connec to the server." error
...
class PermissionsController: UIViewController {
    ...
    let oauth = OAuth2ClientCredentials(settings: [
        ...
        "authorized_uri": "https://api.yelp.com/oauth2/token",
        "secret_in_body": true,
        "keychain": false
        ])
    ...
    func requestOAuthToken() {
        ...
        oauth.authorize { authParams, error in
            ...
            if let params = authParams {
                ...
                let alertController = UIAlertController(title: "Request Error", message: "Authorization was cancelled or went wrong: \(error!)", preferredStyle: .alert)
